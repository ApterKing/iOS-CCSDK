...
static NSString *kCCKeyboardHelperTapGusture= @"kCCKeyboardHelperTapGusture";
static NSString *kCCKeyboardHelperDismissType= @"kCCKeyboardHelperDismissType";
static NSInteger tapTag = 2014;

static NSString *kCCKeyboardHelperRootViewOriginalOriginY = @"kCCKeyboardHelperRootViewOriginalOriginY";
static NSString *kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary = @"kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary";
static NSString *kCCKeyboardHelperRootViewSortedInputFieldArray = @"kCCKeyboardHelperRootViewSortedInputFieldArray";

static NSString *kCCKeyboardHelperDoneBlock = @"kCCKeyboardHelperDoneBlock";

static NSString *kCCKeyboardHelperValidationType = @"kCCKeyboardHelperValidationType";
static NSString *kCCKeyboardHelperInputFiedlTextLength = @"kCCKeyboardHelperInputFiedlTextLength";
static NSString *kCCKeyboardHelperTextLengthLimit = @"kCCKeyboardHelperTextLengthLimit";
...
@interface CCKeyboardHelperTapGusture: UITapGestureRecognizer
@property (assign, nonatomic) NSInteger tag;
@end
@implementation CCKeyboardHelperTapGusture
@end


@interface CCKeyboardHelper()
...
- (UIToolbar *)createCCKeyboardHelperToolBar
{
...
- (void)initilize
{
    ...
    //tool bar
    self.toolBar = [self createCCKeyboardHelperToolBar];
    
    self.toolBarForCleanMaskView = [self createCCKeyboardHelperToolBar];
...
{
    static CCKeyboardHelper *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[CCKeyboardHelper alloc]init];
    });
    return instance;
}
...
- (void)setupCCKeyboardHelperForView:(UIView *)view withDismissType:(CCKeyboardHelperDismissType)dismissType doneBlock:(CCKeyboardHelperDoneBlock)doneBlock
{
    ...
    if (getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewOriginalOriginY) == nil) {
        setAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewOriginalOriginY, @(_currentRootView.frame.origin.y));
    }
    
    setAssociatedObjectForKey(view, kCCKeyboardHelperDismissType, @(dismissType));
    
    NSMutableDictionary *dic= [NSMutableDictionary new];
    setAssociatedObjectForKey(view, kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary, dic);
    
    NSMutableArray *array = [NSMutableArray new];
    setAssociatedObjectForKey(view, kCCKeyboardHelperRootViewSortedInputFieldArray, array);
    
    objc_setAssociatedObject(view, (__bridge  const void *)(kCCKeyboardHelperDoneBlock), doneBlock, OBJC_ASSOCIATION_COPY_NONATOMIC);
    
    [self checkInputFieldInView:view withViewOriginY:view.frame.origin.y];
    
    
    
    NSArray *keys = [getAssociatedObjectForKey(view, kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary) allKeys];
    if (keys.count == 0) {
        return;
    }
    ...
    if (dismissType == CCKeyboardHelperDismissTypeNone) {
        
        _toolBar.frame = _defaultToolBarFrame;
        
    } else if (dismissType == CCKeyboardHelperDismissTypeCleanMaskView){
        
        inputAccessoryView = _cleanMaskView;
        
    } else if (dismissType == CCKeyboardHelperDismissTypeTapGusture){
        
        setAssociatedObjectForKey(view, kCCKeyboardHelperTapGusture, kCCKeyboardHelperTapGusture);
    }
    
    for (NSNumber *key in sortedOriginYArray){
        
        UIView *inputField = [getAssociatedObjectForKey(view, kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary) objectForKey:key];
        
        [getAssociatedObjectForKey(view, kCCKeyboardHelperRootViewSortedInputFieldArray) addObject:inputField];
...
{
    [self setupCCKeyboardHelperForView:view withDismissType:dismissType doneBlock:nil];
}
...
- (void)setupValidationType:(CCValidationType)type forInputField:(UIView *)inputField
{
    ...
    setAssociatedObjectForKey(inputField, kCCKeyboardHelperValidationType, @(type));
    
    switch (type) {
        case CCValidationTypeNumberInt:
        {
            [self setKeyBoardType:UIKeyboardTypeNumberPad forInputField:inputField];
        }
            break;
        case CCValidationTypePhone:
        {
...
{
    setAssociatedObjectForKey(inputField, kCCKeyboardHelperTextLengthLimit, @(length));
}
              ...
              withViewOriginY:(CGFloat)y
{
    ...
    for (UIView *subView in view.subviews){
        ...
        if ([subView isKindOfClass:[UITextField class]] ||
            ...
            NSMutableDictionary *dic = getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewAllInputFieldOriginYDictionary);
            [dic setObject:subView forKey:key];
            
        } else {
            [self checkInputFieldInView:subView
                        withViewOriginY:y + subView.superview.frame.origin.y];
        }
    }
}
...
{
    for (UIView *tf in getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewSortedInputFieldArray)) {
...
- (void)didClickButtonPrevious
{
    ...
    NSArray *sortedInputFields = getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewSortedInputFieldArray);
...
- (void)didClickButtonDone
{
    ...
    frame.origin.y = [getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperRootViewOriginalOriginY) floatValue];
    ...
    CCKeyboardHelperDoneBlock doneBlock = getAssociatedObjectForKey(self.currentRootView, kCCKeyboardHelperDoneBlock);
    if (doneBlock) {
        //callback what??
        doneBlock(nil);
    }
...
{
    if (inputField) {
        NSInteger dismissType = [getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperDismissType) integerValue];
        
        UIToolbar *toolBar = dismissType == CCKeyboardHelperDismissTypeCleanMaskView ? _toolBarForCleanMaskView : _toolBar;
...
- (void) keyboardWillShow:(NSNotification *) notif
{
    ...
    NSString *str = getAssociatedObjectForKey(_currentRootView, kCCKeyboardHelperTapGusture);
    
    BOOL hasAddGusture = NO;
    for(id gusture in _currentRootView.gestureRecognizers){
        if ([gusture isKindOfClass:[CCKeyboardHelperTapGusture class]] && ((CCKeyboardHelperTapGusture *)gusture).tag == tapTag) {
            hasAddGusture = YES;
            break;
        }
    }
    
    if ([str isEqualToString:kCCKeyboardHelperTapGusture] && !hasAddGusture) {
        CCKeyboardHelperTapGusture *tap = [[CCKeyboardHelperTapGusture alloc]initWithTarget:self action:@selector(didClickButtonDone)];
        tap.tag = tapTag;
...
- (void) keyboardWillHide:(NSNotification *) notif
{
    ...
    if ([str isEqualToString:kCCKeyboardHelperTapGusture]) {
        for(id gusture in _currentRootView.gestureRecognizers){
            if ([gusture isKindOfClass:[CCKeyboardHelperTapGusture class]] && ((CCKeyboardHelperTapGusture *)gusture).tag == tapTag) {
...
- (UITextRange *)getMarkedTextRange:(UIView *)inputField
{
    if ([inputField isKindOfClass:[UITextField class]]) {
        return ((UITextField *)inputField).markedTextRange;
    } else if([inputField isKindOfClass:[UITextView class]]){
        return ((UITextView *)inputField).markedTextRange;
    } else if([inputField isKindOfClass:[UISearchBar class]]){
        return nil;
    }
    return nil;
...
- (void)checkText:(NSString *)text forInputField:(UIView *)inputField validation:(NSString *)validation
{
    ...
    NSInteger preLength = [getAssociatedObjectForKey(inputField, kCCKeyboardHelperInputFiedlTextLength) integerValue];
    NSLog(@"current text %@ len %lul, prelen %ld",text,(unsigned long)[text length], (long)preLength);
    ...
    if (![self isTextAvailable:text forValidation:validation]) {
        ...
        setAssociatedObjectForKey(inputField, kCCKeyboardHelperInputFiedlTextLength, @(preLength));
    } else {
        setAssociatedObjectForKey(inputField, kCCKeyboardHelperInputFiedlTextLength, @(text.length));
    }
...
{
    NSInteger type = [getAssociatedObjectForKey(inputField, kCCKeyboardHelperValidationType) integerValue];
    
    //length limit
    NSInteger limitLength = [getAssociatedObjectForKey(inputField, kCCKeyboardHelperTextLengthLimit) integerValue];
    if (limitLength) {
        UITextRange *markedTextRange = [self getMarkedTextRange:inputField];
        NSString *text = [self getTextForInputField:inputField];
        if (markedTextRange == nil && text.length && text.length > limitLength) {
    ...
    if (text.length) {
        ...
        switch (type) {
            case CCValidationTypeNone:
            {
                //do nothing
            }
                break;
            case CCValidationTypeNoWhiteSpace:
            {
            ...
            case CCValidationTypeNumberInt:
            {
                ...
                break;
            case CCValidationTypeAlphabetAndNumber:
            {
...
@implementation NSString (CCKeyboardHelper)
