...
{
    return [NSString decodeBase64String:self encoding:encoding];
}

+ (NSString *)decodeBase64String:(NSString *)input encoding:(NSStringEncoding)encoding
{
    NSData *data = [input dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    data = [CCGTMBase64 decodeData:data];
    NSString *base64String = [[NSString alloc] initWithData:data encoding:encoding];
    return base64String;
}
...
{
    CFStringRef escaped = CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)self, nil, nil, kCFStringEncodingUTF8);
